...
class AuthService {
  ...
  Future<void> init() async {
    ...
    try {
      final user = await fetchCurrentUserModel();
      _currentUser = user;
  ...
  Future<String?> register({
    ...
    final userMap = {
      ...
      'fullName': fullName,
      'password': password, // per instruksi tim (plaintext)
    };
  ...
  /// Login manual: search user by email in Firestore and compare password
  Future<String?> login({required String email, required String password}) async {
    ...
    if (storedPassword == null) throw Exception('Password tidak ditemukan pada record user');
    if (storedPassword != password) throw Exception('Password salah');
